package solutions.sulfura.hyperkit.utils.serialization.jackson3.alias.deserialization;
...
import solutions.sulfura.hyperkit.utils.serialization.jackson3.alias.serialization.AliasBeanPropertyWriter;
...
public class AliasBeanDeserializer extends BeanDeserializer {
    ...
    @Override
    public Object deserialize(JsonParser parser, DeserializationContext deserializationContext) {
        ...
        for (String propName = parser.nextName(); propName != null; propName = parser.nextName()) {
            parser.nextToken();
    ...
    private ResolvedProperty resolveProperty(String propertyName, DtoProjection<?> projection) {
        ...
        if (fieldConfDataForAlias == null || fieldConfDataForAlias.fieldConf() == null) {
            return new ResolvedProperty(_beanProperties.findDefinition(propertyName), null);

        }

        String resolvedPropertyName = fieldConfDataForAlias.fieldName();
        SettableBeanProperty prop = _beanProperties.findDefinition(resolvedPropertyName);
